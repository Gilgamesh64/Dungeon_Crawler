...
void combat_menu() {
    ...
    do {
        ...
        player_damage += apply_sword(room);
...
bool dragon_combat() {
    ...
    if (is_in == !select_option(text, "SI", "NO", NULL)) {
        ...
        printf("Non hai indovinato! Il drago ti attacca\n");
    }
    return false;
}
...
void boss_menu() {
    ...
    for (int i = 0; i < 5; i++) {
        int selection = select_option("Scegli la tua azione: ", "Scudo", "Magia", "Spada", NULL) + 1;
        ...
        if (shield_magic_sword(selection)) {
            ...
            printf("Hai perso questo round contro il Signore Oscuro!\n");
        }
        if(wins >=3) {
            break;
        }
    ...
    if(wins >=3) {
        printf("Complimenti! Hai sconfitto il Signore Oscuro e completato il gioco!\n");
    } else {
        printf("Purtroppo non sei riuscito a sconfiggere il Signore Oscuro. Riprova!\n");
    }
}

bool shield_magic_sword(int player_choice) {
    // shield = 1, magic = 2, sword = 3
    int computer_choice = (roll_dice()+1) / 2;
    ...
    if (player_choice == computer_choice) {
        printf("Pareggio! Rigioca il round!\n");
        return shield_magic_sword(player_choice);
    }
